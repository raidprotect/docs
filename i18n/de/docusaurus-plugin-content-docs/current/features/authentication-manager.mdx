...
Der Authentication Manager (AM) von RaidProtect ist ein fortschrittliches Authentifizierungssystem, das den Zugang zu bestimmten Discord-Rollen durch eine Identitätsverifizierung schützt. Dank Passkeys (WebAuthn), PIN-Codes und OTP können nur authentifizierte Mitglieder [sensible Rollen](/de/learn/least-privilege) erhalten.

## ❓ Wie funktioniert der Authentication Manager? {#working}
...
Es gibt drei Methoden: den PIN-Code (einfach oder verstärkt), das OTP (2FA-Code nach dem TOTP-Standard) und die Passkeys (WebAuthn). Passkeys bieten das höchste Sicherheitsniveau (Grad S).

### Was passiert, wenn eine Sitzung abläuft?

Die geschützte Rolle wird dem Mitglied automatisch entzogen. Es muss sich über `/auth` erneut authentifizieren, um sie wieder zu erhalten, oder seine Sitzung verlängern, bevor sie abläuft.

### Was tun, wenn ich meine Anmeldedaten vergesse?

Über Ihr Profil `/auth-profile` setzt die Schaltfläche „Ich habe meine Anmeldedaten vergessen" Ihr Konto vollständig zurück: Alle Ihre Methoden werden gelöscht und Ihre Zugänge widerrufen. Diese Aktion ist unwiderruflich und erfordert eine vollständige Neukonfiguration.

### Wie viele Rollen kann man mit dem Authentication Manager schützen?

Sie können 3 Rollen in der kostenlosen Version und 10 Rollen in der Premium-Version schützen, für jeweils 20 bzw. 50 Benutzer.
