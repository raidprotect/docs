...
- Choice of [sanctions](./features/anti-spam.md#triggers) applied by the anti-spam. <SuggestionLink id="85" />
- New anti-spam logs (better visibility on detections).
- Enable [minimum account age]((./features/raid-mode#minage)) without captcha. <SuggestionLink id="314" />
- Complete redesign of the [`/userinfo` command](./features/utilities.mdx#userinfo).
- New logs for major bot configuration changes.
- Fixes:
  - The [`/lock` command](./features/channel-lock.md) now supports forum and media channels, and blocks threads along with channels.
  - Fixed bug in the [`/clear` command](./features/moderation.md#clear) when the bot lacked necessary permissions in a channel.
- Improved UI/UX:
  - Clarified that member, role, and channel select menus allow typing.
  - Check for the `COMMUNITY` feature when configuring Auto Raid Mode.
- Performance and stability:
  - Revamped emoji handling ([`@fca.gg/glyph` v1.0.0](https://github.com/FCAgreatgoals/glyph)).
  ...
  - Integrated Sentry for error tracking and improved developer support ([`@fca.gg/kino` v1.0.0](https://github.com/FCAgreatgoals/kino)).
...
- Added [min-age to the captcha](./features/raid-mode#minage) (ability to set a minimum account age to access the server).
