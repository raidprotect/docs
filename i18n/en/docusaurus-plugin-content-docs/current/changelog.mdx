...
- Selection of [sanctions](./features/anti-spam.md#triggers) applied by anti-spam. <SuggestionLink id="514" />
- New anti-spam logs (better visibility on detections).
- Enforcing a minimum account age without captcha. <SuggestionLink id="314" />
- Blocking/allowing certain file extensions server-wide.
- New logs for important bot configuration changes.
- Fixes:
  - The [`/lock` command](./features/channel-lock.md) now supports forum channels, media channels, and threads.
  - Fixed bug with the [`/clear` command](./features/utilities.md#clear) when the bot lacks necessary permissions in a channel.
- Improved UI/UX:
  - Clarification about typing in member, role, and channel select menus.
