...
## 🏷️ Alterar o pseudónimo de um membro {#nick}

O comando `/nick` permite-lhe alterar o pseudónimo de um membro no servidor, ou repô-lo.

Utilize o comando: ```/nick (membro) [pseudónimo]```

Substitua `(membro)` pela menção ou ID desejado. O parâmetro `[pseudónimo]` é o novo pseudónimo a aplicar (32 caracteres no máximo). Se for omitido, o pseudónimo do membro é reposto.

:::info
O comando requer a permissão do Discord **Gerir pseudónimos** e está também acessível através dos aliases `nickname`, `rename` e `setnick` como [comando por prefixo](../guides/prefix). As alterações são registadas nos logs do servidor.
:::
